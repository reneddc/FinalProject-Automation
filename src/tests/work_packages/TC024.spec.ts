...
test("TC024: Verify work package delete", async ({lateralMenuComponent, workPackagesPage, newPackageFormPage}) => {
  ...
  await workPackagesPage.clickDeleteOption(timeWaiter);
  await workPackagesPage.waitForDeleteModal(timeWaiter);
  await workPackagesPage.clickDeleteConfirmButton();
  const toaster = await workPackagesPage.getSuccessDeleteToaster(timeWaiter);
  expect(toaster).toContainText("Successfully deleted");
});