...
test('TC001: Verify login with invalid account', async ({ loginPage }) => {
  ...
  expect(await loginPage.getErrorMessage()).toContainText('Invalid user or password or the account is blocked due to multiple failed login attempts. If so, it will be unblocked automatically in a short time.');
});
